    ...
    <React.Fragment>
      ...
      >
        ...
        <Box
          component="nav"
          sx={{
            borderTop: '0.0625rem solid var(--MainNav-divider)',
  ...
  return (
    <React.Fragment>
      <Tooltip title="Language">
        ...
        >
          <Box sx={{ height: '1.5rem', width: '1.5rem' }}>
...
function renderNavGroups({ items = [], pathname }: { items?: NavItemConfig[]; pathname: string }): React.JSX.Element {
  ...
  return (
    <Stack component="ul" direction="row" spacing={2} sx={{ listStyle: 'none', m: 0, p: '0.5rem 0.75rem' }}>
      {children}
    </Stack>
  );
}
  ...
  if (items) {
    return (
      <Dropdown>
        ...
        <DropdownPopover
          PaperProps={{ sx: { minWidth: '12.5rem', p: 1 } }}
