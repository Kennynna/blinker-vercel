...
const Wrapper = styled.div`
  ...
  gap: 12px;
  flex: 1;
  height: 96px; // Set a fixed height for the entire component
`;

const ImageWrapper = styled.div`
  flex: 0 0 96px; // Fixed width for the image
  border-radius: 12px;
  overflow: hidden;
  position: relative;
  width: 96px;
...
export function InviteEventCart() {
  return (
    <>
      <Wrapper>
        <ImageWrapper>
        ...
        <TextWrapper>
          ...
          <EventAvatar>
            <Avatar sx={{ bgcolor: 'black', width: '16px', height: '16px' }} />
            <p>Arianna Name</p>
          </EventAvatar>
        </TextWrapper>
      </Wrapper>
      <ButtonGroup>
        <ButtonNo>отклонить</ButtonNo>
        <ButtonYes>
          принять <ArrowRightV2 color="#B7AEAE" />{' '}
        </ButtonYes>
      </ButtonGroup>
    </>
  );
